...
from database.models.models import User, session  # для тестов
...
class CRUDInterface:
    ...
    def insert_single_user(cls,
        ...
        """
        Добавляет пользователя, если такой пользователь
        уже есть то обновляет дату
        :return: User
        """
        if user_id and user_name:
            some_user = User(name=user_name, user_id=user_id)
        else:
            some_user = User()
            some_user.user_id = 123
        try:
            session.add(some_user)
            session.commit()
        except IntegrityError as ex:
            session.rollback()
            some_user = session.query(User).where(User.user_id == some_user.user_id).first()
            some_user.data = datetime.now()
            session.add(some_user)
            session.commit()
        return some_user
...
if __name__ == '__main__':
    print(CRUDInterface.insert_single_user(session,'boris', 124414,))
    # print(CRUDInterface.insert_single_user())
